...
自动点焊系统上位机软件，用于对接 PLC、MES 和本地程序管理流程。当前版本：`v2.21.3`。
...
<AssemblyVersion>2.21.3.0</AssemblyVersion>
<FileVersion>2.21.3.0</FileVersion>
<InformationalVersion>2.21.3</InformationalVersion>
```
...
git checkout main
git merge --no-ff develop -m "release: v2.21.3"
git tag -a v2.21.3 -F tag-notes.txt
git push origin main
git push origin v2.21.3
```

`tag-notes.txt` 为临时文件，内容取自该版本的 CHANGELOG 条目，打完 tag 即可删除。也可用 `git tag -a v2.21.3 -m "标题" -m "正文"` 直接写多段说明。

查看历史版本说明：

```powershell
git tag -n99                # 列出全部 tag 及完整说明
git tag -n99 v2.21.3        # 只看某个版本
git show v2.21.3            # 看 tag 说明 + 指向的提交
```
