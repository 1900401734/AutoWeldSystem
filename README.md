...
- 系统设置：应用全部优先保存本地配置；仅在设备编号、名称或相关服务地址发生变化时后台同步设备信息到 MES，保存和同步结果使用非阻塞全局提示。整件检测设备可将“检测结果来源”切换为 `PLC读取` 或 `程序计算`，存在未完工任务时禁止切换。整件检测还可开启“整件检测四面合并显示”（只影响界面）并选择“A/B配对聚合方式”（影响上传和报表数据，存在未完工任务时禁止切换）。
