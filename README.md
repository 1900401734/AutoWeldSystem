...
自动点焊系统上位机软件，用于对接 PLC、MES 和本地程序管理流程。当前版本：`v2.21.0`。
...
- 系统设置：应用全部优先保存本地配置；仅在设备编号、名称或相关服务地址发生变化时后台同步设备信息到 MES，保存和同步结果使用非阻塞全局提示。整件检测固定走程序判定，“检测结果来源”“A/B配对聚合方式”“整件检测逐面结果显示”“整件检测四面合并显示”四项配置自 `v2.21.0` 起移除；合并视图改由生产监控页「合并显示」复选框控制，默认开启并记住上次选择，仅对拥有对应权限的角色显示（默认仅管理员）。PLC 一组的“判定与上报小数位”（默认 2 位）同时控制**产品判定**、合并视图、上传给 MES 的 XLSX 报表和 MES 过程参数，四者必须同源——操作员在合并视图看到的数值就是判定所用的值；截断还是四舍五入沿用同组的“PLC 数值格式处理方式”。逐面视图、数据管理页“导出测试数据”、历史数据列表和中心看板不受该项影响，始终沿用测试项配置的采集小数位。
...
<InformationalVersion>2.21.0</InformationalVersion>
```
...
git checkout main
git merge --no-ff develop -m "release: v2.21.0"
git tag -a v2.21.0 -F tag-notes.txt
git push origin main
git push origin v2.21.0
```

`tag-notes.txt` 为临时文件，内容取自该版本的 CHANGELOG 条目，打完 tag 即可删除。也可用 `git tag -a v2.21.0 -m "标题" -m "正文"` 直接写多段说明。

查看历史版本说明：

```powershell
git tag -n99                # 列出全部 tag 及完整说明
git tag -n99 v2.21.0        # 只看某个版本
git show v2.21.0            # 看 tag 说明 + 指向的提交
```
