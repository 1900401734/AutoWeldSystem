...
自动点焊系统上位机软件，用于对接 PLC、MES 和本地程序管理流程。当前版本：`v2.12.0`。
...
- 过程参数上传：测试项字典配置单位后，实际值、上限和下限按“值 单位”上传，结果字段保持原值；试焊件字段 `IsTest` 使用 JSON 布尔值 `true/false`；整件检测四面工艺在本地保留 1～4 面原始数据，上传 MES 时输出 `SideNo=A/B` 两条数据，对称度按 A=面2/4、B=面1/3 聚合（聚合方式见“A/B配对聚合方式”设置，默认取最大值），高度取四面最大值并在 A/B 两条数据中重复发送，宽度只取 A 面（面2、面4）最大值、B 条数据的宽度字段传空字符串，`Result` 按配对面从严合并，不发送 `Type`、`TouchNo` 和 `IsTest`。
...
- 系统设置：应用全部优先保存本地配置；仅在设备编号、名称或相关服务地址发生变化时后台同步设备信息到 MES，保存和同步结果使用非阻塞全局提示。整件检测设备可将“检测结果来源”切换为 `PLC读取` 或 `程序计算`，存在未完工任务时禁止切换。整件检测还可开启“整件检测四面合并显示”和“整件检测逐面结果显示”（两者都只影响界面）并选择“A/B配对聚合方式”（影响上传和报表数据，存在未完工任务时禁止切换）。
...
<InformationalVersion>2.12.0</InformationalVersion>
```
