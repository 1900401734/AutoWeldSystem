...
自动点焊系统上位机软件，用于对接 PLC、MES 和本地程序管理流程。当前版本：`v2.18.0`。
...
- 数据管理：使用通用“测试数据”页签兼容点焊与检测设备，按产品展开测试记录树；按任务绑定程序和工位解析对应方案，方案明细中勾选“保存历史”的动态测试项会写入本地历史，并在启用中心同步时转发到中心服务器看板，报表或 MES 独占字段不会展示；原始数据仅供内部流程使用，不在界面展示；报告文件入口独立保留。 当产品工艺仅配置一个采集点且该产品只有一条测试记录时，直接显示为单行；多采集点或重复测试仍保留产品到测试记录的树形结构。 支持按产品结果筛选全部/OK/NG 数据，点击动态测试列按产品升降序排列。导出按**上传报表格式**写入用户指定的 XLSX 路径，并在明细末列追加中文「上传状态」；导出始终覆盖整个工单，不受产品结果筛选和分页影响，动态列与小数位由报表配置决定。 测试数据按产品行分页，默认每页 20 个产品，可切换 20/50/100；产品节点默认折叠，工具栏可一键展开或折叠全部测试记录。 历史工单支持多选删除、清理上传失败工单和按筛选日期区间批量清理，删除会级联清除该工单的采集记录、上传任务、报表记录以及磁盘上已生成的 xlsx 报表；生产中和已暂停的工单会被自动跳过，需要“删除历史数据”权限，删除结果写入操作日志。
- 地址维护：维护固定业务信号对应的 PLC 实际地址，并按工位配置/预览 PLC 配方名称与数字槽位的映射；“启用采集”只控制实时预览，“保存历史”同时决定本地历史展示和中心服务器转发字段，“写入报表”和“上传 MES”可独立配置；各维护页签可按角色独立授权。测试项字典的偏移量表达式支持减法计算式 `(134-84):S-7_3`，用于 PLC 无对应寄存器但可由两个已有偏移相减得出的数值（如焊接前后位移量），只支持一次减法和两个整数相对偏移，不参与本地 OK/NG 判定。
...
<InformationalVersion>2.18.0</InformationalVersion>
```
