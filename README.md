...
- 待上传数据：工单信息页签持续显示未完工任务及开工、过程参数、xlsx 报表和完工上报四阶段状态，直到任务完成且相关上传阶段处理完毕；生产期间可增量生成本地 XLSX，但报告文件必须等待 MES 完工上报成功后才允许上传，启动补传和手动重试同样遵守该顺序；切换各上传页签时会刷新绑定字段元数据，避免有记录但单元格空白。
