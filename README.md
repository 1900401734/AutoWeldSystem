...
自动点焊系统上位机软件，用于对接 PLC、MES 和本地程序管理流程。当前版本：`v2.5.0`。
...
- 数据管理：使用通用“测试数据”页签兼容点焊与检测设备，按产品展开测试记录树；按任务绑定程序和工位解析对应方案，方案明细中勾选“保存历史”的动态测试项会写入本地历史，并在启用中心同步时转发到中心服务器看板，报表或 MES 独占字段不会展示；原始数据仅供内部流程使用，不在界面展示；报告文件入口独立保留。 当产品工艺仅配置一个采集点且该产品只有一条测试记录时，直接显示为单行；多采集点或重复测试仍保留产品到测试记录的树形结构。 支持按产品结果筛选全部/OK/NG 数据，点击动态测试列按产品升降序排列，并将当前筛选和排序后的测试明细导出到用户指定的 XLSX 路径。
