...
public interface IWindowsShellIntegrationService
{
    ...
    /// </summary>
    StartupIntegrationResult ApplyStartupIntegration();

    /// <summary>
    /// 根据指定设置同步开机自启，并确保桌面快捷方式存在。
    /// </summary>
    StartupIntegrationResult ApplyStartupIntegration(AppSettings settings);

    /// <summary>
    /// 启用或关闭当前 Windows 用户登录后的开机自启。
    /// </summary>
    StartupIntegrationResult SyncAutoStart(bool enabled);
