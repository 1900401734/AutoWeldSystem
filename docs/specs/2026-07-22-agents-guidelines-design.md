...
- C# 可空引用类型、命名、异步方法、短方法和简单控制流约定。
- 优先复用现有类、服务、控件和规则，只有至少两个稳定场景需要时才抽取公共抽象。
- 控制台回归测试入口、解决方案构建命令，以及文件锁定时的备用输出目录。
- 配置文件、密码、PLC/MES 地址和本机路径的安全边界。
- 中文 Conventional Commits 规则和 README 按影响更新规则。
- 完成任务后报告复用内容、修改文件、README 判断和验证结果。
...
- 不相关的多种变更拆分为原子提交；提交前检查暂存差异，避免混入无关文件。
